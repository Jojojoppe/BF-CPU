...
ifeq ($(PROGRAMMER), mimasconfig)
prog: $(BINFILE)
	sudo mimasconfig $(MIMAS_PORT) $(BINFILE)
endif
...
# GHDL
###########################################################################
VHDMODULES = $(VHDSOURCE) $(TESTBENCHFILE)
ghdl: $(VHDMODULES)
	-rm -r sim
	-mkdir sim
	ghdl -s --std=93 -Pxilinx-ise/xilinxcorelib/v93 --ieee=synopsys -fexplicit $(VHDMODULES)
	ghdl -a --std=93 -Pxilinx-ise/xilinxcorelib/v93 --ieee=synopsys -fexplicit $(VHDMODULES)
	ghdl -e --std=93 -Pxilinx-ise/xilinxcorelib/v93 --ieee=synopsys -fexplicit $(TESTBENCH)
	ghdl -r --std=93 -Pxilinx-ise/xilinxcorelib/v93 --ieee=synopsys -fexplicit $(TESTBENCH) --vcd=sim/TB.vcd --stop-time=$(STOPTIME)
	mv *.o sim
	mv work* sim
	mv $(shell echo $(TESTBENCH) | tr A-Z a-z) sim
